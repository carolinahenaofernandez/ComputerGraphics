...
void Image::DrawTriangle(const Vector2& p0, const Vector2& p1, const Vector2& p2, const Color& borderColor, bool isFilled, const Color& fillColor) {
    ...
    auto drawLine = [&](const Vector2& start, const Vector2& end, const Color& color) {
        DrawLineDDA(static_cast<int>(start.x), static_cast<int>(start.y),
                    static_cast<int>(end.x), static_cast<int>(end.y), color);
    };

    // Fill the triangle using a scanline algorithm if required
    if (isFilled) {
        auto interpolate = [](float x0, float y0, float x1, float y1) -> std::vector<float> {
            std::vector<float> values;
            if (y0 == y1) return values; // Avoid division by zero for horizontal lines
        ...
        // Interpolate edges
        std::vector<float> x01 = interpolate(v0.x, v0.y, v1.x, v1.y);
        std::vector<float> x12 = interpolate(v1.x, v1.y, v2.x, v2.y);
        std::vector<float> x02 = interpolate(v0.x, v0.y, v2.x, v2.y);

        // Combine edge lists
        std::vector<float> xLeft, xRight;
        int split = x01.size(); // Where x01 ends in x02
        xLeft.insert(xLeft.end(), x01.begin(), x01.end());
        xLeft.insert(xLeft.end(), x12.begin(), x12.end());
        xRight = x02;

        // Fill the triangle
        int yStart = static_cast<int>(v0.y);
        int yEnd = static_cast<int>(v2.y);
        for (int y = yStart; y <= yEnd; ++y) {
            int idx = y - yStart;
            if (idx < static_cast<int>(xLeft.size()) && idx < static_cast<int>(xRight.size())) {
                int xStart = static_cast<int>(std::min(xLeft[idx], xRight[idx]));
                int xEnd = static_cast<int>(std::max(xLeft[idx], xRight[idx]));
    ...
    drawLine(v0, v1, borderColor);
    drawLine(v1, v2, borderColor);
    drawLine(v2, v0, borderColor);
}
