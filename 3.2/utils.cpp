...
bool checkGLErrors()
{
    ...
    while ((errCode = glGetError()) != GL_NO_ERROR) {
        ...
        std::cerr << "OpenGL Error: " << error << " (" << errCode << ")" << std::endl;
...
void DrawLineDDA(Image* framebuffer, Vector2 start, Vector2 end, const Color& c) {
    ...
    if (steps == 0) {
        std::cout << "Skipping line: zero length" << std::endl;
        return;  // Prevent division by zero
    }

    float xIncrement = (end.x - start.x) / (float)steps;
    float yIncrement = (end.y - start.y) / (float)steps;
    ...
    for (int i = 0; i <= steps; i++) {
        if (x >= 0 && x < framebuffer->width && y >= 0 && y < framebuffer->height) {
            ...
            std::cout << "Pixel set at (" << round(x) << ", " << round(y) << ")" << std::endl;
        } else {
            std::cout << "Skipping out-of-bounds pixel at (" << round(x) << ", " << round(y) << ")" << std::endl;
        }
        x += xIncrement;
        y += yIncrement;
    }
