...
{
    if (particleSystemActive) {
            particleSystem.Update(seconds_elapsed); // Update particles only if active
        }
...
void Application::OnKeyPressed(SDL_KeyboardEvent event)
{
    ...
    case SDLK_p: // Press 'P' to toggle the particle system
...
void ParticleSystem::Init() {
    for (int i = 0; i < MAX_PARTICLES; ++i) {
        particles[i].position = Vector2(rand() % 800, rand() % 600); // Random position within screen bounds
        particles[i].velocity = Vector2((rand() % 20 - 10) / 100.0f, rand() % 50 / 100.0f); // Gentle horizontal drift and slow downward speed
        particles[i].color = Color(255, 255, 255); // White color for snow
        particles[i].ttl = 5.0f + (rand() % 200) / 100.0f; // Time-to-live between 5-7 seconds
        particles[i].inactive = false; // Activate particle
    }
}
...
void ParticleSystem::Render(Image* framebuffer) {
    for (int i = 0; i < MAX_PARTICLES; ++i) {
        if (!particles[i].inactive) {
            int size = 1 + rand() % 3; // Snowflake size: 1 to 3 pixels
            for (int dx = 0; dx < size; ++dx) {
                for (int dy = 0; dy < size; ++dy) {
                    framebuffer->SetPixel(particles[i].position.x + dx, particles[i].position.y + dy, particles[i].color);
                }
            }
        }
    }
...
void ParticleSystem::Update(float dt) {
    for (int i = 0; i < MAX_PARTICLES; ++i) {
        if (particles[i].inactive) continue; // Skip inactive particles

        // Update position based on velocity
        particles[i].position.x += particles[i].velocity.x * dt * 100.0f; // Gentle horizontal drift
        particles[i].position.y += particles[i].velocity.y * dt * 100.0f; // Downward movement

        // Decrease TTL and deactivate if expired
        particles[i].ttl -= dt;
        if (particles[i].ttl <= 0.0f || particles[i].position.y > 600) {
            // Reset particle to the top of the screen
            particles[i].position = Vector2(rand() % 800, 0); // Reset at a random X position, top of screen
            particles[i].velocity = Vector2((rand() % 20 - 10) / 100.0f, rand() % 50 / 100.0f); // Random drift and downward speed
            particles[i].ttl = 5.0f + (rand() % 200) / 100.0f; // Reset TTL
            particles[i].inactive = false; // Reactivate particle
        }
    }
}
